...
public class FaceServer implements IFaceServer{
    ...
    public FaceServer(final boolean isGpu, final String modelDir, final int port, final boolean isDebug) throws IOException, SQLException {
        ...
        String modelPath = modelDir +  "model";
        ...
        try(ZMQ.Socket socket = this.zContext.createSocket(ZMQ.REP))  {
            ...
            while (!Thread.currentThread().isInterrupted()) {
                ...
                try {
                    ...
                    String loggedMessage = "TODO: Log failed messages for analysis later.";
                    log.error("Message failed. Exception is: " + ex);
                    ex.printStackTrace();
                    log.error("Logged message to: " + loggedMessage);
                }
    ...
    private float[] imageToFeatures(final RecognitionMessage message){
        ...
        try {
            ...
            final BufferedImage faceImage = this.faceDetector.detect(inputImage, 112);
            if(faceImage != null){
    ...
    public static void main(final String[] args) {
        ...
        try
        {
            ...
            modelPath = resourceLoader.getResource(modelPath).getURI().getPath();
